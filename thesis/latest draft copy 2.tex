...
The threshold $\tau = 2$ consecutive failures was implemented in the reference implementation as:
...
We first evaluate a baseline configuration where each agent executes the standard inverse-kinematics controller from Chapter~\ref{chap:closed_loop_fault_adaptation} with no access to the shared failure ledger. In the reference implementation, this is captured as:
...
Faults are simulated as encoder slip with Gaussian-distributed magnitude, as implemented in the simulation framework:
