...
\label{subsubsec:architecture_details}
...
\label{subsubsec:design_rationale}
...
\subsubsection{Design Rationale}
\label{subsubsec:design_rationale_1}
...
\label{subsubsec:layer_definitions}
...
\label{subsubsec:transposed_convolution_mathematics}
...
\label{subsubsec:batch_normalization}
...
\label{subsubsec:design_rationale_for_four_layers}
...
\label{subsubsec:bilinear_upsampling}
...
\label{subsubsec:alternative_why_not_transpose_to_100_times_100_directly}
...
\label{subsec:total_parameter_count}
...
The dataset comprises 2,000 samples (1,600 training, 400 validation) generated using identical Lorenz trajectories as in Chapter~\ref{chap:baseline_latency_bottleneck}. The end-effector workspace spans $x \in [0.05, 0.25]$ m, $y \in [-0.15, 0.10]$ m, $z \in [0.00, 0.30]$ m.
...
\label{subsubsec:positional_encoding_2d}
...
\label{subsubsec:spatialconvstream_encoder}
...
\label{subsubsec:feature_fusion_and_mlp_head}
...
\label{subsubsec:weighted_joint_loss}
...
\label{subsubsec:hyperparameter_selection}
...
\label{subsubsec:why_no_curriculum_learning_or_weighted_loss}
...
Figure~\ref{fig:end_effector_correlation} shows the correlation between true and predicted end-effector positions for the NeuroKin-3D model. The tight diagonal clustering is consistent with the mean error of 2.39 mm reported in our empirical evaluations.
...
\begin{enumerate}
    ...
    \item \textbf{Stereoscopic decoding recovers accurate 3D}: NeuroKin-3D achieves 2.39 mm mean spatial error in our empirical evaluations.
\end{enumerate}
...
The \texttt{factory\_state} dictionary serves as the stigmergic communication channel—the shared environment that agents modify and respond to. Its design embodies three key properties essential for effective temporal stigmergy as defined in the reference implementation within the simulation framework.

\paragraph{Simplicity:} The state contains only two variables: \texttt{consecutive\_failures} (integer) and \texttt{mode} (derived, not stored). The reference implementation shows in Code~\ref{code:minimal_state}:
...
The threshold $\tau = 2$ consecutive failures was implemented in Code~\ref{code:recovery_threshold}:
...
We first evaluate a baseline configuration where each agent executes the standard inverse-kinematics controller from Chapter~\ref{chap:closed_loop_fault_adaptation} with no access to the shared failure ledger. In the reference implementation, this is captured as shown in Code~\ref{code:baseline_call}:
...
The NeuroKin configuration, implemented as \path{run_single_robot_chain_faulty_neurokin_v2} in the simulation framework, equips each agent with:
...
The reference implementation shows agents reading the ledger at initialization, as shown in Code~\ref{code:ledger_read}:
...
Then executing the reaching task, and writing their outcome back as shown in Code~\ref{code:ledger_write}:
...
These results are already summarized in Table~\ref{tab:swarm_results}.
...
Critically, the NeuroKin success rate (88\%) is measured \textit{post-fault}—that is, after fault injection has begun. This matches exactly the 88\% reported in the simulation framework for the NeuroKin variant under fault conditions. For comparison, the baseline's pre-fault success rate (before any faults occur) is 100\%, but this drops to 13\% post-fault.

The reference implementation confirms that recovery mode activates based on the shared state, as shown in Code~\ref{code:recovery_activation}:
...
Faults are simulated as encoder slip with Gaussian-distributed magnitude, as shown in Code~\ref{code:fault_injection}:
...
\begin{enumerate}
    ...
    \item \textbf{Agents can write diagnostic state to a shared ledger}: The \texttt{factory\_state} dictionary is updated with task outcome in real time, as shown in the simulation framework.
    ...
    \item \textbf{This adaptation improves collective outcomes}: 88\% success vs. 13\% baseline post-fault, as reported in the simulation framework.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Rapid self-diagnosis}: NeuroKin-3D provides end-effector position estimates with 2.39 mm mean error (from Section~\ref{subsec:neurokin3d}), enabling real-time failure detection.
